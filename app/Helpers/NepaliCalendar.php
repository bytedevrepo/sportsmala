...
class NepaliCalendar
{
    ...
    public function eng_to_nep($yy, $mm, $dd)
    {
        ...
        else
        {
            ...
            $this->_nep_date['year']    = $this->eng_to_nep_number($y);
            $this->_nep_date['month']   = $m;
            $this->_nep_date['date']    = $this->eng_to_nep_number($total_nDays);
    ...
    public function eng_to_nep_number($number)
    {
        $arr1 = str_split($number);
        $returnDate = '';
        foreach($arr1 as $num) {
            $returnDate .= $this->convert($num);
        }
        return $returnDate;
    }
