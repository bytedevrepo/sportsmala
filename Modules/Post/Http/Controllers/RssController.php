...
//        dd($request->all());
        ...
        if(@simplexml_load_file($feed->feed_url)):
            ...
            if(!empty($feeds)):
                ...
                foreach ($feeds->channel->item as $key => $item) :
                    ...
                    $assetUrl = $this->getImg($item , $namespaces);
                    $test = parse_url($assetUrl, PHP_URL_PATH);

                    if($assetUrl!=""){
                        if(preg_match('/\.(jpg|png|jpeg|PNG|JPEG|JPG)/', $assetUrl)){
                            $post->post_type        = 'article' ;
                            $post->image_id = $this->imageUpload( $assetUrl);
                        }
                        elseif(preg_match('/\.(mp4|3gp|webm)/', $assetUrl)){
                            $post->post_type        = 'video' ;
                            $post->video_url_type   = 'mp4_url' ;
                            $post->video_url = $this->character_convert($assetUrl);
                        }
                    }
    ...
    public function getImg($item , $namespaces){
        ...
        if(@$item->coverImages):
            return(((string)$item->coverImages));
        elseif(@$item->image):
    ...
    public function imageUpload($request)
    {
        try {
            ...
            $path      = parse_url($requestImage, PHP_URL_PATH);       // get path from url
