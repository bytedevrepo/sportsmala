        ...
        <div class="thumb">
            @if($content['text-image-text'][1]['image'] != "")
                @if(isFileExist($content['text-image-text'][1]['image'] , $result =  $content['text-image-text'][1]['image']->medium_image_three))
                    {{--<img src="{{safari_check() ? basePath(@$content['text-image-text'][1]['image']).'/'.$result : static_asset('default-image/default-255x175.png') }} " --}}
                         {{--data-original=" {{basePath($content['text-image-text'][1]['image'])}}/{{ $result }} " class="img-fluid"   alt="{!! $post->title !!}"  >--}}
                    <figure class="album__thumb">
                        <img src="{{ basePath(@$content['image-text'][0]['image']).'/'.$result }}"
                             alt="{!! $post->title !!}">
                    </figure>
                @else
